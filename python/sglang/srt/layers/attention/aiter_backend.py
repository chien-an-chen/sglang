...
class AiterAttnBackend(AttentionBackend):
    ...
    def __init__(
        ...
        if self.use_mla:
            ...
            # The DCP decode kernel tiles the query heads, so it serves any local
            # head count; the check below is for the non-DCP path only.
            assert (
    ...
    def _mla_decode_fwd_with_head_pad(
        ...
        **kwargs,
        ...
        kernel"), which is why every count below 16 is brought up to it here.

        Under repeat the real heads stay at [0:num_head] and the rest are
        ...
        if self.head_pad_mode == "repeat" or (
            ...
            if return_lse:
                ...
                return o[:, : self.num_head, :], lse[:, : self.num_head]
        ...
        if self.head_pad_mode == "zero":
            ...
            if return_lse:
                ...
                return o[:, :num_head, :], lse[:, :num_head]
            mla_decode_fwd(q_in, k_buffer_flat, o, **kwargs)
            return o[:, :num_head, :]
        o = q.new_empty(
